...
| v0.6 Package/Release Readiness | Current | Target version is `0.6.0a0`; local release smoke has proved the wheel exposes `ai-workbench-bootstrap`, bootstraps a fresh repo, and renders missing-evidence PR gate recovery before any TestPyPI or PyPI upload |
...
| PyPI and package plumbing | v0.6 release prep | Historical package build checks, wheel smoke, TestPyPI install, exact-version PyPI install, and MCP Registry publication passed for `0.2.0a0`; `0.3.0a0` build, wheel smoke, TestPyPI upload/install, PyPI upload/install, and MCP Registry publication passed. `0.6.0a0` local build, twine check, fresh-wheel bootstrap smoke, TestPyPI upload, and TestPyPI exact-version install proof passed; PyPI and MCP Registry remain approval-gated |
