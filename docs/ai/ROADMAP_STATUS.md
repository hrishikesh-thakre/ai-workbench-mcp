...
| v0.6 Package/Release Readiness | Current | Target version is `0.6.0a0`; local release smoke, TestPyPI proof, and PyPI proof have verified `ai-workbench-bootstrap`, fresh-repo bootstrap, idempotent `runs/`, and missing-evidence PR gate recovery |
...
| PyPI and package plumbing | v0.6 release prep | Historical package build checks, wheel smoke, TestPyPI install, exact-version PyPI install, and MCP Registry publication passed for `0.2.0a0`; `0.3.0a0` build, wheel smoke, TestPyPI upload/install, PyPI upload/install, and MCP Registry publication passed. `0.6.0a0` local build, twine check, fresh-wheel bootstrap smoke, TestPyPI upload/install, and PyPI upload/install passed; MCP Registry remains approval-gated |
