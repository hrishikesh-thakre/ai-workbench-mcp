...
class PublicHygieneTests(unittest.TestCase):
    ...
    def test_v06_release_note_does_not_claim_published_package(self) -> None:
        metadata = json.loads(SERVER_JSON.read_text(encoding="utf-8"))
        pyproject = tomllib.loads(PYPROJECT.read_text(encoding="utf-8"))
        readme_text = README.read_text(encoding="utf-8")
        ...
        self.assertIn("Publication status: TestPyPI upload and exact-version install proof completed on", v06_release_text)
        self.assertIn("Do not", v06_release_text)
        self.assertIn("explicit release approval", v06_release_text)
        self.assertIn("`ai-workbench-mcp==0.6.0a0` is the target package version", readme_text)
        self.assertIn("latest historical published package before this release is `ai-workbench-mcp==0.3.0a0`", readme_text)
