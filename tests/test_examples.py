...
class PublicExamplesTests(unittest.TestCase):
    ...
    def test_sample_runs_readme_and_analytics_guide_document_phase5_reports(self) -> None:
        ...
        docs_only_routing_design_text = DOCS_ONLY_ROUTING_DESIGN.read_text(encoding="utf-8")
        ...
        self.assertIn("docs/routing/docs-only-current-tier-policy-design.md", targeted_report_text)
        self.assertIn("docs/routing/docs-only-current-tier-policy-design.md", roadmap_text)
        self.assertIn("Docs-only current-tier advisory policy", roadmap_text)
        self.assertIn("Selector feedback now returns `prefer_current_tier` only", roadmap_text)
        self.assertIn("docs_only_current_tier_when_accepted", docs_only_routing_design_text)
        self.assertIn("implemented as advisory selector behavior", docs_only_routing_design_text)
        self.assertIn("Implemented Behavior", docs_only_routing_design_text)
        self.assertIn("workbench-docs-only-acceptance.yaml", docs_only_routing_design_text)
        self.assertIn("`docs_only`", docs_only_routing_design_text)
        self.assertIn("`local_coding`", docs_only_routing_design_text)
        self.assertIn("`overall_status=\"passed\"`", docs_only_routing_design_text)
        self.assertIn("`sign_off_ready=true`", docs_only_routing_design_text)
        self.assertIn("`final_status=\"accepted\"`", docs_only_routing_design_text)
        self.assertIn("do not change selector default rules", docs_only_routing_design_text)
        self.assertIn("do not auto-promote or auto-demote model tiers", docs_only_routing_design_text)
        self.assertIn("risk is `medium` or `high`", docs_only_routing_design_text)
        self.assertIn("evidence source is PR-gate fallback scaffold evidence", docs_only_routing_design_text)
        self.assertIn("missing or fallback PR scaffold evidence never counts", docs_only_routing_design_text)
        ...
        self.assertIn("latest historical published package before this release is", pypi_text)
        ...
        self.assertIn("TestPyPI upload and exact-version install proof also completed on", pypi_text)
        ...
        self.assertIn("Do not rerun the upload for `0.3.0a0` or `0.6.0a0`.", pypi_text)
        self.assertIn("TestPyPI dry run completed for `ai-workbench-mcp==0.3.0a0`", pypi_text)
        self.assertIn("https://test.pypi.org/project/ai-workbench-mcp/0.3.0a0/", pypi_text)
        ...
        self.assertIn("PyPI upload for `ai-workbench-mcp==0.6.0a0` is pending explicit release", pypi_text)
        self.assertIn("PyPI release completed for `ai-workbench-mcp==0.3.0a0`", pypi_text)
        self.assertIn("https://pypi.org/project/ai-workbench-mcp/0.3.0a0/", pypi_text)
        ...
        self.assertIn("version `0.6.0a0` is pending PyPI publication and explicit approval", pypi_text)
