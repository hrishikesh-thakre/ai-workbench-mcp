...
from ..evidence_gate.gate_engine import AcceptanceReport
...
def extract_what_worked(report: AcceptanceReport, evidence_root: Path) -> str:
    ...
    if closeout:
        ...
        return " ".join(line for line in lines if not line.startswith("#") and line.strip())
...
def extract_tags(report: AcceptanceReport) -> str:
    tags = ["ai-workbench", "captured", report.task_type]
...
def capture_from_json_report(json_path: str, evidence_path: str, output_dir: str) -> Path:
    ...
    from ..evidence_gate.gate_engine import Decision
    from ..evidence_gate.types import CheckResult, Issue, Severity
