"""
Codex hook adapter — normalizes Codex hook stdin into ai-workbench event schema,
applies PreToolUse guards, and captures evidence.

Usage as a Codex hook:
    python -m ai_workbench_mcp.supervisor.codex_hooks
...
READ_ONLY_SHELL_WRITE_ACTIONS = {"warn", "block"}
...
def normalize_codex_event(raw: dict) -> dict:
    """Normalize a Codex hook stdin object into the ai-workbench event schema.
    ...
    return {
        "schema": "ai-workbench.codex-hook-event.v1",
        "type": "hook_event",
...
def _normalize_read_only_shell_write(value: str | None) -> str:
    normalized = str(value or "warn").strip().lower()
    return normalized if normalized in READ_ONLY_SHELL_WRITE_ACTIONS else "warn"
...
def check_file_edit(
    ...
    file_path: str,
    task_type: str,
    read_only_shell_write: str = "warn",
) -> list[dict]:
    ...
    if tool_name == "bash" and _has_file_write_command(command):
        severity = "blocker" if _normalize_read_only_shell_write(read_only_shell_write) == "block" else "warning"
        violations.append({
            "severity": severity,
...
def evaluate_pre_tool_use(
    ...
    violations.extend(check_file_edit(
        tool_name,
        command,
        file_path,
        task_type,
        read_only_shell_write=read_only_shell_write,
    ))
...
def _spool_fail_open(reason: str, raw: str = "") -> None:
    ...
    try:
        ...
        print(f"ai-workbench hook fail-open not spooled: {reason}", file=sys.stderr)


def _hook_policy_for_cwd(cwd: str) -> tuple[str, str]:
    state = load_state()
    _project_key, record = find_project_for_cwd(cwd, state)
    if record:
        hook_policy = record.get("hook_policy") or {}
        read_only_shell_write = (
            os.environ.get("AI_WORKBENCH_READ_ONLY_SHELL_WRITE")
            or hook_policy.get("read_only_shell_write")
            or "warn"
        )
        return (
            record.get("task_type") or os.environ.get("AI_WORKBENCH_TASK_TYPE", "audit"),
            _normalize_read_only_shell_write(read_only_shell_write),
        )
    return (
        os.environ.get("AI_WORKBENCH_TASK_TYPE", "audit"),
        _normalize_read_only_shell_write(os.environ.get("AI_WORKBENCH_READ_ONLY_SHELL_WRITE")),
    )
...
def run_codex_hook() -> int:
    ...
    Environment variables:
        AI_WORKBENCH_EVIDENCE_DIR  — legacy direct hook capture directory (default: ./runs/codex-hooks)
        AI_WORKBENCH_TASK_TYPE     — task type for policy resolution (default: audit)
    ...
    evidence_dir = os.environ.get("AI_WORKBENCH_EVIDENCE_DIR", os.path.join(os.getcwd(), "runs", "codex-hooks"))
    task_type, read_only_shell_write = _hook_policy_for_cwd(normalized.get("cwd", os.getcwd()))

    violations = evaluate_pre_tool_use(
        normalized,
        task_type,
        read_only_shell_write=read_only_shell_write,
    ) if _is_pre_tool_use(normalized) else []
    decision, reason = compute_decision(violations)

    try:
        spool_event(normalized, violations, decision, reason)
    except Exception as exc:
        print(f"ai-workbench hook spool failure: {exc}", file=sys.stderr)

    if os.environ.get("AI_WORKBENCH_LEGACY_EVIDENCE_CAPTURE"):
        try:
