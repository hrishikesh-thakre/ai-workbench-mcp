...
def get_state_dir() -> Path:
    override = (
        os.environ.get("AI_WORKBENCH_DAEMON_HOME")
        or os.environ.get("AI_WORKBENCH_STATE_DIR")
        or os.environ.get("AI_GATE_DAEMON_HOME")
        or os.environ.get("AI_GATE_STATE_DIR")
    )
    if override:
    ...
    if os.name == "nt":
        ...
        if base:
            return Path(base) / "ai-workbench"

    xdg = os.environ.get("XDG_CONFIG_HOME")
    if xdg:
        return Path(xdg) / "ai-workbench"
    return Path.home() / ".config" / "ai-workbench"
...
def get_spool_dir() -> Path:
    override = os.environ.get("AI_WORKBENCH_SPOOL_DIR") or os.environ.get("AI_GATE_SPOOL_DIR")
    if override:
        return Path(override).expanduser().resolve()
...
def default_evidence_root(project_dir: str) -> str:
    return str(Path(project_dir) / "runs")
