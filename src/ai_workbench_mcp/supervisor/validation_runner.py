...
VALIDATION_MISSING = "AI_WORKBENCH_VALIDATION_MISSING"
VALIDATION_TIMEOUT = "AI_WORKBENCH_VALIDATION_TIMEOUT"
VALIDATION_FAILED = "AI_WORKBENCH_VALIDATION_FAILED"
VALIDATION_PASSED = "AI_WORKBENCH_VALIDATION_PASSED"
...
def _not_required(kind: str, task_type: str) -> ValidationResult:
    return ValidationResult(
        kind=kind,
        command=None,
        status="not_required",
        exit_code=None,
        output=(
            f"No conservative {kind} command was auto-detected for task_type={task_type}.\n"
            "This check is recorded as not required for the supervised public alpha gate.\n"
        ),
        skipped_reason="not auto-detected",
    )
...
def run_validation(
    ...
    for kind in ("test", "lint", "typecheck"):
        ...
        if task_type not in CODE_CHANGE_TASK_TYPES:
            ...
            results[kind] = _missing(kind) if kind == "test" else _not_required(kind, task_type)
        else:
