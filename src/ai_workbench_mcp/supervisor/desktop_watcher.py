...
final Workbench acceptance checks at session end.

Usage:
    ai-workbench opencode-watch [--project-dir .] [--task-type audit]
"""
...
class OpenCodeWatcher:
    ...
    def __init__(
        ...
        self._evidence_dir = os.path.join(self.project_dir, "runs", "opencode-watch")
        self._event_count = 0
    ...
    def _write_closeout(self, db: sqlite3.Connection) -> None:
        ...
        closeout = (
            ...
            f"Confidence: 0.85 — live-watched by AI Workbench ({self.task_type}).\n"
    ...
    def _run_gate(self) -> Optional[int]:
        """Run final Workbench acceptance checks. Returns exit code or None if skipped."""
        ...
        try:
            report = run_acceptance(
                ...
                evidence_path=self._evidence_dir,
                transcript_path=transcript_path,
                policy_dir=str(_POLICY_DIR),
                task_type=self.task_type,
            )
            write_report(report, self._evidence_dir)
            write_csv_report(report, self._evidence_dir)
            return {
                "ACCEPT": 0,
                "REJECT": 1,
                "ACCEPT_WITH_CONDITIONS": 2,
                "BLOCKED": 3,
            }.get(report.decision.value, 1)
    ...
    def start(self) -> None:
        ...
        print("=== AI Workbench Desktop Watcher ===")
                ...
                try:
                    ...
                    for ev in events:
                        ...
                        for v in violations:
                            ...
                            raise_alert(
                                f"AI Workbench: {sev.upper()} Violation",
                                msg,
                                severity=sev,
                            )
                            if sev == "high" and self.auto_terminate:
                                print("    Auto-terminating OpenCode...")
                                terminate_opencode()
