...
from .report_browser import build_report_index, canonical_state_for_run
...
LEGACY_PROJECT_STATE_MAP = {
    "BLOCKED": "block",
    "REJECT": "block",
    "ACCEPT_WITH_CONDITIONS": "needs_review",
    "ACCEPT": "accept",
}
LOCK_STALE_SECONDS = 120
STARTUP_FILENAME = "ai-workbench-daemon.cmd"
...
class SupervisorDaemon:
    ...
    def _acquire_lock(self) -> None:
        ...
        if lock:
            ...
            if not same_owner and not self._lock_is_stale(lock):
                ...
                raise RuntimeError(f"ai-workbench supervisor is already running (pid {lock.get('pid')}).")
            if not same_owner:
                self._log(
    ...
    def _finalize_project(self, project_key: str, reason: str = "finalize") -> dict[str, Any] | None:
        ...
        canonical_artifacts_present = (
            (builder.run_path / "validation_report.json").is_file()
            and (builder.run_path / "revision_decision.json").is_file()
        )
        reused = bool(
            existing_metadata.get("finalized_at")
            and existing_metadata.get("decision")
            and canonical_artifacts_present
        )
        if reused:
        ...
        canonical = canonical_state_for_run(builder.run_path, metadata)
        record.update({
            "current_run_id": "",
            "current_run_path": "",
            "current_agent": "",
            "current_session_id": "",
            "latest_decision": canonical.get("decision") or metadata.get("decision", ""),
            "latest_run_path": str(builder.run_path),
            "latest_agent": builder.agent,
            "latest_session_id": builder.session_id,
            "latest_next_action": canonical.get("required_next_action") or metadata.get("required_next_action", ""),
            "latest_updated_at": utc_now(),
        ...
        self._log(
            ...
            run_path=str(builder.run_path),
            decision=canonical.get("decision") or metadata.get("decision", ""),
...
def _codex_hooks_configured(project_dir: str) -> bool:
    ...
    try:
        ...
        return False
    return "ai_workbench_mcp.supervisor.codex_hooks" in text
...
def _project_next_action(state: str, coverage: str, existing: str) -> str:
    if existing:
        return existing
    if state == "RUNNING":
        return "Wait for the daemon to finalize the current run, or run ai-workbench supervisor stop to finalize now."
    if state == "block":
        return "Resolve blockers in the latest Workbench report, then run a new supervised session."
    if state == "needs_review":
        return "Review the latest Workbench findings before treating the run as accepted."
    if state == "FALLBACK_ONLY":
        return "Run ai-workbench setup codex, trust hooks through /hooks, and collect a hook-observed run."
    if state == "UNSUPERVISED" and coverage == "HOOKS_CONFIGURED_UNTRUSTED_UNVERIFIED":
        return "Start or restart Codex, open /hooks, review and trust the ai-workbench hook, then run a session."
    if state == "UNSUPERVISED" and coverage == "OPENCODE_SQLITE_COMPATIBLE":
        return "Start the daemon and run an OpenCode Desktop session in this registered project."
    if state == "UNSUPERVISED":
        return "Register the project and configure a supported agent hook or OpenCode daemon supervision."
    return ""


def _normalize_project_state(value: object) -> str:
    state = str(value or "")
    if state in CANONICAL_PROJECT_STATES:
        return state
    return LEGACY_PROJECT_STATE_MAP.get(state, "")


def _opencode_coverage(record: dict[str, Any], opencode_health: dict[str, Any]) -> str:
    if "opencode" not in record.get("agents", []):
        return ""
    last_seen = record.get("last_seen", {}).get("opencode", {}) or {}
    if (
        record.get("current_agent") == "opencode"
        or record.get("latest_agent") == "opencode"
        or last_seen.get("last_event_at")
        or last_seen.get("last_rowids")
    ):
        return "OPENCODE_SQLITE_OBSERVED"
    if opencode_health.get("compatible"):
        return "OPENCODE_SQLITE_COMPATIBLE"
    return ""


def _combined_coverage(
    record: dict[str, Any],
    opencode_coverage: str,
    codex_coverage: str,
) -> str:
    agent = record.get("current_agent") or record.get("latest_agent") or ""
    if agent == "opencode" and opencode_coverage:
        return opencode_coverage
    if agent == "codex" and codex_coverage:
        return codex_coverage
    if opencode_coverage == "OPENCODE_SQLITE_OBSERVED":
        return opencode_coverage
    if codex_coverage in {"HOOKS_OBSERVED", "FALLBACK_ONLY"}:
        return codex_coverage
    if codex_coverage == "HOOKS_CONFIGURED_UNTRUSTED_UNVERIFIED":
        return codex_coverage
    if opencode_coverage == "OPENCODE_SQLITE_COMPATIBLE":
        return opencode_coverage
    return codex_coverage or opencode_coverage or "UNSUPERVISED"
...
def status_rows(
    ...
    for key, record in sorted(state.get("projects", {}).items()):
        ...
        opencode_coverage = _opencode_coverage(record, opencode_health)
        codex_coverage = ""
        ...
        if record.get("current_run_path"):
            ...
            canonical = canonical_state_for_run(run_path, metadata) if run_path else {}
            latest_decision = canonical.get("decision") or record.get("latest_decision") or ""
            normalized_decision = _normalize_project_state(latest_decision)
            if normalized_decision:
                project_state = normalized_decision
        ...
        coverage = _combined_coverage(record, opencode_coverage, codex_coverage)
        next_action = _project_next_action(
            project_state,
            coverage,
            str(canonical.get("required_next_action") or record.get("latest_next_action", "")),
        )
        rows.append({
            "key": key,
            "project_dir": record.get("project_dir", ""),
            ...
            "decision": "" if project_state == "RUNNING" else project_state,
            "supporting_acceptance_decision": metadata.get("supporting_acceptance_decision", ""),
            "latest_run_path": run_path,
            "required_next_action": next_action,
            "next_action": next_action,
            "opencode_coverage": opencode_coverage,
...
def _startup_command_text() -> str:
    ...
    return (
        ...
        f'"{sys.executable}" -m ai_workbench_mcp.cli supervisor start >> "{log_path}" 2>&1\r\n'
    )
...
def _validate_startup_python() -> None:
    result = subprocess.run(
        [sys.executable, "-c", "import ai_workbench_mcp.cli"],
        capture_output=True,
        text=True,
        timeout=15,
    )
    if result.returncode != 0:
        raise RuntimeError("Unable to import ai_workbench_mcp.cli with the current Python executable.")
