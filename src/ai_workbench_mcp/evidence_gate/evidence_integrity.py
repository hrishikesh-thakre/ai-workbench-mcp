...
def _is_supervisor_run(evidence_root: Path) -> bool:
    metadata = _read_json(evidence_root / "metadata.json") or {}
    return metadata.get("schema") == "ai-workbench.run.v1"
...
def check_evidence_depth(evidence_path: str) -> CheckResult:
        ...
        if isinstance(files, list) and len(files) > 0:
            ...
            if has_sizes < len(files) * 0.5:
                issues.append(Issue(
                    "evidence_depth",
                    Severity.WARNING,
                    "Less than 50% of inventory entries have size_bytes — inventory may be superficial",
                ))
    ...
    if closeout:
        lines = [line for line in closeout.splitlines() if line.strip() and not line.strip().startswith("#")]
        if len(lines) < 3:
            issues.append(Issue(
                "evidence_depth",
                Severity.WARNING,
...
def check_git_status_authenticity(evidence_path: str, transcript_path: str) -> CheckResult:
    ...
    if status_before_path.is_file() and status_after_path.is_file() and not _is_supervisor_run(Path(evidence_path)):
